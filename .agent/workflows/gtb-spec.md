...
3. **Draft the spec**:
   ...
   - In the **Testing Strategy** section, explicitly evaluate whether the feature warrants E2E BDD scenarios (Godog). TDD and BDD methodologies are a key requirement of any spec and must be put front and centre as part of the implementation process. Features involving CLI commands, multi-step user workflows, or service lifecycle coordination should include Gherkin feature files in `features/`. Reference the suitability assessment in `docs/development/specs/2026-03-28-godog-bdd-strategy.md` for guidance on when BDD adds value versus when standard Go tests suffice. 
