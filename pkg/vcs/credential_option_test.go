...
func TestCredentialOption_HandsTheFactoryGTBsChain(t *testing.T) {
	...
	o := forge.NewOptions(CredentialOption(forge.Endpoint{Type: forge.SourceTypeGitLab}, cfg, "GITLAB_TOKEN"))
...
func TestCredentialOption_AnAbsentSectionStillReadsTheFallbackVariable(t *testing.T) {
	...
	o := forge.NewOptions(CredentialOption(forge.Endpoint{Type: forge.SourceTypeGitea}, cfg, "GITEA_TOKEN"))
...
func TestCredentialOption_ARefusedRungIsAnErrorNotAnAbsence(t *testing.T) {
	...
	o := forge.NewOptions(CredentialOption(forge.Endpoint{Type: forge.SourceTypeGitHub}, cfg, "GITHUB_TOKEN"))
	require.NotNil(t, o.Credential)
...
// TestCredentialOption_BitbucketKeepsTheFactorysComposition: Bitbucket
// authenticates with two halves the adapter composes from its own keys, and
// GTB's chain knows only auth.*, so the option carries nil, which the factory
// reads as its default (spec 0025 D3, OQ1).
func TestCredentialOption_BitbucketKeepsTheFactorysComposition(t *testing.T) {
	t.Parallel()

	cfg := ConfigFromReader(testutil.ViewFromYAML(t, "bitbucket:\n  username: me\n  app_password: secret\n"))
	o := forge.NewOptions(CredentialOption(forge.Endpoint{Type: forge.SourceTypeBitbucket}, cfg, ""))

	assert.Nil(t, o.Credential)
}
