...
func DetachSign(signer crypto.Signer, publicKey []byte, data io.Reader, sigCreationTime time.Time) ([]byte, error) {
	...
	// Name the local to match the field it feeds (the previous
	// `deterministic := false` read as the inverse of its own value,
	// audit: detachsign-buffering-and-inverted-flag-name).
	...
	// Sign straight into the armor encoder — there is no need to buffer the
	// raw signature packet and io.Copy it across afterwards (audit:
	// detachsign-buffering-and-inverted-flag-name).
	...
	if err := openpgp.DetachSign(enc, entity, bytes.NewReader(body), cfg); err != nil {
