...
- **The provider factories do use it**, and GTB cannot stop them: `forge.ProviderFactory` takes a config and no credential. So `vcs.ConfigFromReader` is the forge-facing view: it presents the credential GTB's chain resolves as `auth.value` and shows `auth.env`/`auth.keychain` as unset, because a factory that finds them beside an empty `auth.value` reports the configuration as stale and fails construction (#76 broke every consumer's update check in CI, where the shipped `<forge>.auth.env` default was the only auth key present). GTB's own chain reads the raw view underneath (`readerFor` unwraps it), so `doctor` still names `auth.env` as the winning rung.
