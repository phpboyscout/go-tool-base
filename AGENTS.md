...
- Address the root cause first; do not silence a linter to avoid fixing real issues. When a suppression is **genuinely unavoidable** (e.g. gosec G204 for an intentional dev-tooling subprocess, G304 for an operator-named file path), use a **narrowly-scoped inline `//nolint:<linter> // <justification>`** on the exact line, naming only enabled linters (`nolintlint` fails the build on a directive that suppresses nothing) — never a file-scoped `.golangci.yaml` exclusion, which would also mask new, unintended violations of that linter in the same file.
...
Configuration is the extracted `gitlab.com/phpboyscout/go/config` Store: layered precedence (changed CLI flags > env vars > project-local `.<tool>.yaml` > config files > tool `ConfigPaths` assets > merged `assets/config.yaml` embedded defaults), snapshot-pinned reads via `store.View()`, transactional in-place writes via `Apply`, and explicit hot-reload via `Store.Watch` (wired by the root pre-run). A config file's format comes from its extension; YAML is built in and the seven other `go/config` formats are links (`pkg/config/formats/<format>`, blank-imported from `main`, contributing a codec under `setup.SlotConfigCodec`), so an unlinked format's extension is refused and no framework package outside those links may import a format module (`pkg/props/config_formats_footprint_integration_test.go`). Config sources (spec 0204) are named slots in `props.Tool.Config.Sources`, placed in `Config.Layers`; a kind is a link package calling `setup.RegisterConfigSourceKind`, and the root builds the store in two passes so a factory reads its `config.sources.<name>` settings from defaults, files, env and flags only, never the project file (`pkg/cmd/root/config_sources.go`). The tool's own file is in `props.Tool.Config.Format` (yaml, toml, json or hcl) and named `props.Tool.ConfigFilename()`; init templates stay YAML and `init` encodes on write (spec 0204 R4). That order is the default: a tool may declare its own in `props.Tool.Config.Layers`, lowest first, with `defaults` lowest, `flags` highest and `project` below `env` enforced by `props.New` and the store (spec 0204). Embedded defaults always apply and are segregated per owning package/feature bundle — see [`0138-segregated-default-config`](https://gitlab.com/phpboyscout/go-tool-base/-/wikis/specs/0138-segregated-default-config).
...
The `mcp` command is the estate's [`go/mcp`](https://mcp.go.phpboyscout.uk) module, wired through `pkg/mcp` (spec 0201). `pkg/mcp.NewCmdMCP` applies GTB's conventions once: exposure from `setup.IsExposedToMCP` (spec 0089) with pure groups never published, the global flags `--config`/`--debug`/`--ci`/`--accessible` withheld, operations grouped by the `setup.Wrap` feature ID, and the publication mode from `props.Tool.MCP` (`properties.mcp.mode` in the manifest: compact by default, `direct` for one native tool per command). Every call runs the command as a subprocess of the same binary. A command declares its MCP tool annotations with `setup.AnnotateMCP` (built-ins already do); a generated command records them as `mcp_hints` via `gtb annotate`. Nothing in the framework imports `go/mcp` outside `pkg/mcp`, and `pkg/mcp` is the **link** that gives a tool the feature (spec 0202): a blank import from `main` (`cli/cmd/gtb/mcp.go`; a generated project's `cmd/<name>/mcp.go`, written or removed from the manifest's `mcp` entry, which defaults on) declares `mcp` as a link kind and contributes the command through `setup.SlotRootCommand`. The root never names the package; a binary without the import has no `mcp` command and carries neither `go/mcp` nor the SDK (`pkg/props/mcp_footprint_integration_test.go` guards it). The command's log level is `Props.LogLevel`; the command is stamped `setup.MarkProtocolStdout` so the root renders no prompt under it. The wizard asks the publication mode on the MCP page and, on `gtb wizard`, which commands stay on the surface.
...
Credential storage is the extracted `gitlab.com/phpboyscout/go/credentials` module (with the opt-in `go/credentials/keychain` backend). User-supplied secrets (AI API keys, VCS tokens, Bitbucket app passwords) are stored via one of three modes selected by the setup wizard: env-var reference (recommended default), OS keychain (opt-in blank import of `go/credentials/keychain`), or literal in config (legacy). Literal mode is refused under `CI=true`. Resolution precedence at runtime: `{provider}.api.env` or `auth.env` → env var → `{provider}.api.keychain` or `auth.keychain` → `{provider}.api.key` or `auth.value` literal → well-known fallback env var. The `doctor` command's `credentials.no-literal` check warns when any literal credential is present in config. Keychain mode is activated by a blank import of `gitlab.com/phpboyscout/go-tool-base/pkg/setup/keychain` in the tool's `main` (see `cli/cmd/gtb/keychain.go`); it links the `go/credentials/keychain` backend and declares the `keychain` feature as a link kind (`props.KindLink`, spec 0199), so `doctor` and the generator's catalogue know the binary carries it. Regulated downstreams omit the import, and linker dead-code elimination keeps go-keyring and its transitive deps out of the linked binary. See the `go/credentials` and `go/credentials/keychain` modules, and [`0054-credential-storage-hardening`](https://gitlab.com/phpboyscout/go-tool-base/-/wikis/specs/0054-credential-storage-hardening).
...
- **The provider factories are handed the chain, and compose none of their own.** `forge.ProviderFactory` takes `forge.WithCredential` (go/forge v0.30.0, spec 0025), and a factory given a source consults it and nothing else, so `auth.env`/`auth.keychain` beside an empty `auth.value` can no longer read as stale. Every construction site (`pkg/setup/update.go`, `pkg/setup/forge/profile.go`, `pkg/setup/forge/ssh.go`) passes `vcs.CredentialOptions(endpoint, cfg, fallbackEnv)...`, which wraps `ForgeCredential` over the endpoint's section; the construction context bounds the resolution and a rung that refuses fails construction with its reason. `vcs.ConfigFromReader` is a plain view again (it used to resolve the chain into `auth.value` on `context.Background()` and swallow the error, #76/#78). Bitbucket gets both halves (`forge.WithUsername` and `forge.WithCredential`, go/forge spec 0026, #89): `pkg/vcs/bitbucket.go` walks each half's env-reference and literal keys and the shared keychain blob, read once.

`doctor` reports resolution as well as storage: a per-forge `<Forge> credential` check names the winning rung (`auth.env`, `auth.keychain`, `auth.value`, or the fallback variable) and never the value, and diagnoses a configured-but-broken credential rather than reporting it as absent. Note that every forge bundle ships `<forge>.auth.env: <FORGE>_TOKEN` as a default, so rung 1 already reads the variable the fallback rung would — the fallback is effectively shadowed for shipped forges, and `GITHUB_TOKEN` reports as `resolves from auth.env`. Which config layers a tool wires, and in what order, is declarable via `props.Tool.Config.Layers` (lowest precedence first; the order is the precedence, spec 0204 D1); unstated means the framework default, and the keychain is not among them. See [`0183-forge-credentials-and-configurable-config-layers`](https://gitlab.com/phpboyscout/go-tool-base/-/wikis/specs/0183-forge-credentials-and-configurable-config-layers).
...
Releases use the **Release-MR** pattern via [colophon](https://gitlab.com/phpboyscout/colophon), the estate's own release orchestrator, included as the `gitlab.com/phpboyscout/cicd/colophon` CI component — **do not manually tag**. On merges to `main`, colophon opens/updates a Release MR from the `colophon/release/main` branch containing the pending version bump and `CHANGELOG.md` entries. Merging that Release MR (the human gate) resolves the commit that actually landed on `main`, tags it `vX.Y.Z`, and the tag pipeline runs `goreleaser` — darwin/linux/windows × amd64/arm64, CGO disabled, FIPS — which uploads the binaries, SBOMs and signatures to the release store at `pkg.phpboyscout.uk/go-tool-base/<tag>/`; colophon then creates the GitLab Release on that tag, linking every uploaded asset **after verifying each link resolves**, so a release never exists with nothing on it. colophon owns the changelog/notes, GoReleaser owns the artefacts. macOS binaries are notarized; a Homebrew **cask** (`homebrew_casks:` in `.goreleaser.yaml`, not a formula) is auto-updated on the GitLab-hosted tap at `gitlab.com/phpboyscout/homebrew`, pushed over SSH with a deploy key because GoReleaser's `token:` field is GitHub-only. Users install it with `brew install --cask gtb` after tapping that URL.
...
Pre-release check: run `just ci`, then `just goreleaser-check`, then `just snapshot` to verify `dist/` output. Both run goreleaser **Pro** (`goreleaser-pro`): `.goreleaser.yaml` carries the static release channel's `before_publish` hook (spec 0203), which OSS goreleaser refuses to parse, and the tag pipeline runs Pro too. gtb self-updates from that channel (`release_source.type: static`, base `https://pkg.phpboyscout.uk/go-tool-base`): each tag publishes `release.json` beside its binaries and moves the `latest.json` pointer last; see `docs/reference/static-release-channel.md`.
