...
- **The provider factories are handed the chain, and compose none of their own.** `forge.ProviderFactory` takes `forge.WithCredential` (go/forge v0.30.0, spec 0025), and a factory given a source consults it and nothing else, so `auth.env`/`auth.keychain` beside an empty `auth.value` can no longer read as stale. Every construction site (`pkg/setup/update.go`, `pkg/setup/forge/profile.go`, `pkg/setup/forge/ssh.go`) passes `vcs.CredentialOption(endpoint, cfg, fallbackEnv)`, which wraps `ForgeCredential` over the endpoint's section; the construction context bounds the resolution and a rung that refuses fails construction with its reason. `vcs.ConfigFromReader` is a plain view again (it used to resolve the chain into `auth.value` on `context.Background()` and swallow the error, #76/#78). Bitbucket is the exception: its two halves are the adapter's own composition, so the option carries nil there.
