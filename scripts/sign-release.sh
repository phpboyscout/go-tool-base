...
public_key="${GTB_SIGNING_KEY_PUBLIC:-cli/pkg/trustkeys/keys/signing-key-v2.asc}"
...
if [[ ! -f "${public_key}" ]]; then
	...
	echo "  (embedded at cli/pkg/trustkeys/keys/signing-key-v2.asc by default)." >&2
	exit 1
fi
