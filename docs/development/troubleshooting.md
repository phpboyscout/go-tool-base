...
- **"OS keychain" option missing from the `init ai` wizard.** Expected when the current host has no reachable [credentials `Backend`](../explanation/components/credentials.md). `KeychainAvailable()` returning `true` but `Probe()` returning `false` means the backend is compiled in but unreachable: on Linux the usual cause is a session bus without a registered Secret Service provider (GNOME Keyring / KWallet). See [Testing the keychain on a headless host](testing/headless-keychain-testing.md) for diagnostics and workarounds (`dbus-run-session + gnome-keyring-daemon`, containerised Secret Service, or the in-memory backend swap).
- **`ErrCredentialUnsupported` from `credentials.Store` / `Retrieve` / `Delete`.** The stub backend is active: your tool's `main` hasn't blank-imported `gitlab.com/phpboyscout/go-tool-base/pkg/setup/keychain` (the framework's link) and hasn't registered a custom `Backend` via `credentials.RegisterBackend`. Add the blank import (`gtb enable keychain` writes `cmd/<tool>/keychain.go`) and rebuild, or register your own backend: see [the custom-backend how-to](../how-to/custom-credential-backend.md).
