...
**Mitigation.** Introduced [`pkg/regexutil`](../explanation/components/regexutil.md) with `CompileBounded` and `CompileBoundedTimeout` helpers enforcing a 1 KiB length cap and a 100 ms wall-clock compile timeout. Both affected call sites route through the helper. Tool authors accepting patterns in their own config should use the same helper — see [the component doc](../explanation/components/regexutil.md#call-site-discipline) and `CLAUDE.md` § Regex Compilation.
