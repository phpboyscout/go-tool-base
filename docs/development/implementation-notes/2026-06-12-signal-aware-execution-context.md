...
1. **E2E Gherkin SIGINT smoke scenario** (spec verification item 3). Not added in this MR: Godog steps would need to spawn the e2e binary as a real subprocess, deliver SIGINT, and assert exit code 130 + no orphaned temp files. The unit tests cover the lifecycle deterministically; the E2E adds OS-level confidence (especially around `signal.Notify` registration). Want it as a follow-up, or in this MR?
2. **TUI coexistence integration test.** D5 is satisfied by analysis of Bubble Tea v2's documented raw-mode behaviour (and its own `handleSignals` comment). A real integration test would need a PTY harness (e.g. `creack/pty`) to drive a huh prompt and inject a literal `^C` byte. Is the analysis sufficient, or should a PTY-based test be scheduled?
...
4. **Exit message wording.** An interrupted run logs `ERROR interrupted by signal: interrupt` (through the ErrorHandler at fatal level) before exiting 130. kubectl prints nothing extra. Happy with the explicit line, or prefer a quieter exit?
