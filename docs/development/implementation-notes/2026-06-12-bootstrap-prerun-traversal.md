...
- No E2E Gherkin scenario added. The spec's verification plan item 4 mentions an
  E2E check on a scaffolded tool; the unit + ordering regression tests cover the
  behavioural guarantee directly. Flagged below as a question.

## Questions for Matt

1. Do you want an E2E BDD scenario (verification-plan item 4: scaffolded tool
   with a custom subcommand hook resolving config correctly), or are the unit
   regression + ordering tests sufficient for this fix?
