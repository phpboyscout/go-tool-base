...
| `just test-e2e` | Godog / BDD scenarios against the `cli/cmd/e2e` binary |
...
## E2E test binaries: `cli/cmd/e2e` vs `cli/cmd/gtb`
...
> **The gotcha that keeps biting us:** because `cli/cmd/e2e` *enables* `InitCmd`, the framework bootstrap **requires a config file** (init is how a tool's first config gets created). Scenarios that drive `cli/cmd/e2e` must therefore provide one. The CLI step world writes a default `config.yaml`, and `aTemporaryDirectoryWithConfigFile` overrides it per scenario. Run `cli/cmd/e2e` with no config and it exits non-zero with *"no configuration files found, please run init, or provide a config file"*.

### `cli/cmd/gtb`: the real shipped binary

`cli/cmd/gtb` (`cli/pkg/cmd/root`) **disables** `InitCmd`, so its bootstrap treats a missing config as *allow-empty* and gtb's own command-line tooling (`generate`, `regenerate`, `remove`, `keys`) runs **without any config file**, exactly as it does in production.

Scenarios that test **gtb's own tooling** must build and run `cli/cmd/gtb` (via `support.GeneratorBinaryPath()`), **not** `cli/cmd/e2e`. Driving `generate`/`add-flag` through the `InitCmd`-enabled `cli/cmd/e2e` would wrongly demand a config file (green on a developer's machine that has an ambient `~/.config/gtb` config, red in CI's clean environment) and misrepresents how the generator actually runs.

### Rule of thumb

| Testing… | Binary | Helper |
|----------|--------|--------|
| A **framework** feature a downstream tool inherits (init, doctor, config, update, chat, controls, signals) | `cli/cmd/e2e` | `support.BinaryPath()` |
| **gtb's own** tooling *behaviour* (generate/regenerate/remove output, add-flag metadata, key files) | `cli/cmd/gtb` | `support.GeneratorBinaryPath()` |
...
> **One deliberate exception.** `cli/cmd/e2e` *does* register the generator commands (`cli/cmd/e2e/main.go`), but only so that **input-validation rejection** scenarios (`features/generator/validation.feature`, e.g. `generate project --name BadName` is rejected) can run through the shared CLI step-world, which supplies a config. Those scenarios assert *rejection* and never reach real generation, so the InitCmd/config requirement is met by the CLI world's config. Anything exercising **successful generation behaviour** must still use `cli/cmd/gtb`.
...
- [Manual credential testing](manual-credentials.md): walk through the OS-keychain storage mode end-to-end using the `cli/cmd/e2e` binary: wizard UX, runtime resolution, CI refusal, probe gating, Bitbucket JSON blob, and regulated-build stripping.
