...
| **Release source** | The tool can build its updater. On a forge, the release source type has a registered provider (a failure names the module to blank-import). On the static channel ([spec 0203](https://gitlab.com/phpboyscout/go-tool-base/-/wikis/specs/0203-the-static-release-channel)) the check reads the pointer at the base URL: a pass names the current tag and how many downloads it lists; nothing published yet is a warning naming the pointer URL; a pointer whose manifest does not resolve fails with both URLs. Skipped when self-update is disabled or no release source is set |
