...
| `--release-channel` | *(`forge` when hosted)* | Where self-update releases from when `update` is enabled. `forge` reads the backend's releases and needs a hosted project. `static` reads a pointer and per-tag manifests under `--release-base-url` with no forge involved ([spec 0203](https://gitlab.com/phpboyscout/go-tool-base/-/wikis/specs/0203-the-static-release-channel)); it is the one channel a `--no-forge` project can take. `direct` was withdrawn and is refused by name. Recorded as `release_source.type`. |
| `--release-base-url` | | The static channel's location: an `https` URL with no userinfo, query or placeholder host, under which the release publishes `latest.json` and `<tag>/release.json` ([layout](../static-release-channel.md)). Required with `--release-channel static`, refused with any other. Recorded as `release_source.static.base_url`; `gtb set release_source.static.base_url` changes it (set the URL before switching the type). |
