...
| `keychain` | on | `cmd/<name>/keychain.go` blank-importing `pkg/setup/keychain`, the framework's link, which activates the OS keychain backend and declares the feature. `gtb disable keychain` removes the file for a regulated build (no go-keyring / godbus / wincred linked). See [How to configure credentials](../../../../how-to/configure-credentials.md) and [How to implement a custom credential backend](../../../../how-to/custom-credential-backend.md). |
