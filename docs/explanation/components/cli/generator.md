...
The `cli/pkg/generator` package is the core engine responsible for all code generation, project scaffolding, and AST manipulation in `gtb`. This document provides a deep technical dive into the architecture for contributors.
...
The seed's departure from "a present line is left alone" is the modules the
generator owns. The framework itself is the first floor: the regenerated code
is written against the running gtb's version, so a `go-tool-base` line below
it is raised (a v0.42.0 project regenerated by a newer gtb otherwise failed
typecheck on the framework symbols the new templates use until the author ran
`go get` by hand). `gomod.LockstepFloors` then marks each forge and chat
adapter, keychain and signing link the running gtb knows a version for as a
floor, so a line below it is raised on regenerate (spec 0200 D9). The adapters implement the
...
1.  Update `cli/pkg/generator/manifest.go` to support the new type in the `ManifestFlag` struct.
2.  Update `cli/pkg/generator/manifest_update.go` if the new type affects the `ManifestCommandUpdate` struct or `updateCommandRecursive` logic.
3.  Update `cli/pkg/generator/templates/command.go` to map the type to the corresponding Cobra method (e.g., `Flags().DurationVar`).
4.  Update `cli/pkg/generator/ast.go` if the flag needs to be injectable into existing ASTs (complex types might need special handling).
