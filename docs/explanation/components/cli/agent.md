...
The `cli/pkg/agent` package defines the capabilities and environment for the **Autonomous Repair Agent**. This agent is responsible for the self-healing code generation feature, where AI-generated code is iteratively fixed until it compiles and passes tests.
...
The `cli/pkg/generator` package orchestrates this agent.
...
Create a new file `cli/pkg/agent/search_tool.go` or add to `tools.go`.
...
Update `cli/pkg/agent/ai.go` (or wherever the agent is initialized) to include your new tool in the definition list.
...
1.  **Define the Tool**: Add a new function definition in `cli/pkg/agent/tools.go`.
