...
**Adding a forge** means four things: a blank import of its adapter module in `cli/cmd/gtb/providers.go` (and `cli/cmd/e2e`), a `Profile`, an embedded config bundle, and an entry in `pkg/setup/forge`'s provider-to-module table (`ModuleFor`), which a guard test holds against the profile set so the linked-adapter hint can never name a forge without an import path. Everything that enumerates forges (the feature registry, the doctor support bundle, the project generator's backend chooser) derives from those.

A fourth is needed to make it *scaffoldable*: an entry in the generator's feature catalogue (`cli/pkg/generator/templates/feature_catalogue.go`), carrying the constant's declaring package as well as its name. Forge constants live in `pkg/setup/forge`, not `props`, so the emitter qualifies each one against the package recorded in its descriptor: hard-coding `props` is what previously made a selected forge vanish between the manifest and the generated root. A guard test holds the catalogue against the registry, so a new forge fails the build until it is listed.
