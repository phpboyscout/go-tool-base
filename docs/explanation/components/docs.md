...
- **Security headers**: the served site is wrapped with [`SecurityHeadersMiddleware`](http.md#middleware) automatically: one of the few places GTB applies HTTP transport middleware for you (see the [Transport Middleware & Resilience](../concepts/transport-middleware.md) concept).
