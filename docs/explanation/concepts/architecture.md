...
*   **Registry over hard-coding**: The release provider system uses a string-keyed factory registry (now the standalone [`forge`](https://forge.go.phpboyscout.uk) module) so that consuming code — and downstream tools — never import platform packages directly. Each provider ships as its own module and registers itself at `init()`; `pkg/setup/providers.go` blank-imports the first-party set, and a custom provider does the same from `main()` before any update runs. Registering a source type twice is an error rather than a silent overwrite.
