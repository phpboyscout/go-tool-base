...
*   **Registry over hard-coding**: The release provider system uses a string-keyed factory registry (now the standalone [`forge`](https://forge.go.phpboyscout.uk) module) so that consuming code (and downstream tools) never import platform packages directly. Each provider ships as its own module and registers itself at `init()`; the tool's `main` blank-imports the adapters it uses (gtb's own links every first-party forge in `cli/cmd/gtb/providers.go`), and a custom provider does the same before any update runs. Registering a source type twice is an error rather than a silent overwrite.
