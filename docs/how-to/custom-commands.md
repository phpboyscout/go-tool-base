...
- [Command Constructor Pattern](../concepts/command-constructors.md) — rationale for `NewCmd*` and `*setup.Command`.
- [Command Middleware System](../concepts/command-middleware.md) — how `Register` wires global and feature middleware.
- [`gtb generate command`](../cli/generate.md) — the generator emits exactly this shape, so adding commands via the generator is the same as writing them by hand.
- [Migration from v0.4 to v0.5](../migration/v0.4-to-v0.5.md) — diff vs. the old `*cobra.Command` + `AddCommandWithMiddleware` pattern.
